...
\item Any line through the origin is a vector subspace of $\R^n$.
\item Any plane through the origin is a vector subspace of $\R^n$.
\end{itemize}
...
Consider a plane through the origin of $\R^3$. This plane can be described as a two-dimensional subspace, since the plane can be represented by one equation, with fixed $a, b, c \in \R$, $a \neq 0$: 
\[ ax_1 + bx_2 + cx_3 = 0. \]
This one equation only needs two variables to describe its set of vectors. Call this plane P. Then,
\begin{align*}
...
We can decompose this representation of vectors in $P$ by rewriting per variable, getting $P$ as the span of two vectors in $\R^3$. We call these two vectors \textbf{special solutions}. 
\begin{align*}
...
\[ x_1 a_1 + x_2 a_2 + \cdots + x_n a_n = b. \]
In other words, if $b$ is a linear combination of the columns of $A$, then $\vec{x}$ is a solution to $A \vec{x} = b$. 
...
If $\vec{x}_p$ solves $A \vec{x} = b$, we will call $\vec{x}_p$ a \textbf{particular solution} of the equation. 

\vspace{5mm}

By linearity, we have 
\[ A (\vec{x}_n + \vec{x}_p) = A \vec{x}_n + A \vec{x}_p = 0 + b = b. \]
Hence, $\vec{x}_n + \vec{x}_p$ is \emph{also} a solution to $A \vec{x} = b$. 
...
\[ [ A \, | \, 0 ]. \]
Use downward, forward elimination, starting in the upper-left corner. Pivot on the digonal. If you ever zero out a diagonal entry, pivot to the right of that diagonal and continue.

\vspace{5mm}

If you zero out any of the bottom rows, that's okay.
}
...
The bottom left portion of the matrix should only have pivot 1s, and 0s below. The upper right portion ideally has only 0s above pivots; some nonzero entries above pivots may be unavoidable.\footnote{We will see several numerical examples in class.}
...
\end{align*}
There is $r=1$ pivot variable and $n-r=2$ free variables. 
...
\vspace{3mm}
These can both be stated in terms of the matrix $A$ with left-multiplication by the vector $\vec{y}^t$ by applying the transpose: 
...
Why mention $A^t$ in describing how to solve systems using $A$? Because the two systems 
\[ A\vec{x} = b \text{ and } A^t \vec{y} = c \]
are deeply related; in particular, 
\[ \vec{x}^t c = b^t \vec{y}, \text{ or, equivalently, } c^t \vec{x} = \vec{y}^t b. \]
...
Let $V$ and $W$ be two vector subspaces of the same space $Z$ such that $V \cap W = \{0\}$. 

\vspace{5mm}

Then any vector $z \in Z$ has a \emph{unique} representation as a sum $z = v + w$ with $v \in V$ and $w \in W$. When this is the case, we call $V + W$ the \textbf{direct sum} of $V$ and $W$, and denote it $V \oplus W$.
...
The two direct sums above are orthogonal. 
...
We will explore this notion in the next chapter, under the concept of \textbf{projection}. 
