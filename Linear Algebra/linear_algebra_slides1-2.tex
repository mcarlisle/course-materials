...
\title[Intro to Vectors, Solving Linear Equations]{Linear Algebra and Matrix Methods \\ 
...
\begin{itemize}
\item $L$ is \textbf{lower triangular}, with diagonal 1s and 0s above;

\vspace{3mm}

\item $D$ is \textbf{diagonal}, with 0s off the diagonal;

\vspace{3mm}

\item $U$ is \textbf{upper triangular}, with diagonal 1s and 0s below.
\end{itemize}
...
First, we will assume the process works, so there exists a factorization 
\[ A = LDU. \]
If the process fails at any point, then the rows of $A$ must be rearranged, or there is not a unique solution to the system.
...
If $v$ is a row vector, then its \textbf{transpose}\footnote{Sometimes denoted $v^T$... I just went with the lowercase $t$ in these notes.}, denoted $v^t$, is a column vector with the same entries. 
...
\item $A = LDU$ and $A$ is symmetric $\implies$ $U = L^t$ $\implies$ $A = LDL^t$. 
