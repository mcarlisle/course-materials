...
A basis of $N(A^t)$ can be considered the ``missing'' basis vectors from $C(A)$ to span all of $\R^m$. 
...
The union of these two bases is a basis of $\R^n$. 
...
for some $p \in \R^n$. This means $p \in C(P)$. 
...
We can write 
\[ b = p + e = \cvvv{b_1}{0}{b_3} + \cvvv{0}{b_2}{0} \]
...
If 
\[ p = Pb = A\hat{x} \] is the ``best fit'' solution to the attempted \[ A \vec{x} = b, \] with $b = p + e$, and $P$ the projection matrix onto $C(A)$, we have 
\begin{align*}
e & = b - p \\
 & = b - Pb = b - A\hat{x} \\
 & \\
\implies A^t e & = A^t(b - A\hat{x}) \\
 & = A^t b - A^t A \hat{x} = 0 \,\, (\text{since } e \in N(A^t)) \\
 & \\
...
If $\exists x \in \R$ such that $ax = b$, then $b \in C(A)$ and you are done. 
...
\begin{align*}
 & & b - a\hat{x} & = e \\
& \implies & a \cdot (b - a\hat{x}) & = a \cdot e = 0 \,\, (\text{since } a \perp e) \\
...
Let $\vec{x} \in \R^n$ be \emph{any} vector (not necessarily a minimizing one). 

\vspace{5mm}

Given the decomposition $b = p + e$ for $b \in \R^m$, we can write $e$ in terms of $b$, $p$, and \emph{any} $\vec{x} \in \R^n$:  
\begin{align*}
b & = p + e \\
\implies e & = b - p = (A\vec{x} - p) - (A\vec{x} - b), 
\end{align*}
...
This verifies that the least squares solution $\hat{x}$ minimizes the error of any $\vec{x} \in \R^n$: 
\[ ||A\hat{x} - b||^2 = ||e||^2 \leq \inf_{x \in \R^n} ||A\vec{x} - b||^2. \]
...
If we declare that $f$ uses $n+1$ coefficient parameters $c_0, c_1, c_2, ..., c_n$ in its definition, what is the vector of parameters 
\[ c = (c_0, c_1, c_2, ..., c_n) \] 
...
Compute the error $E(c) = ||e||^2$ for a general pair of parameters for the line $f(x) = c_0 + c_1 x$; this yields the square error 
\begin{align*} 
E(c) & = ||Ac - b||^2 \\
...
\[ Q^t Q = I, \]
and we call $Q$ an \textbf{orthogonal matrix}\footnote{Some texts reserve the term \textbf{orthogonal matrix} for square matrices $Q$ only.}. 
...
Note: $Q^t Q = I$, and $Q^t = I - 2u u^t = Q$, so reflection matrices are \textbf{involutions}; they are their own inverses. 
